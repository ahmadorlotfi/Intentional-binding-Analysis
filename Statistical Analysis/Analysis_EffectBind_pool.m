...
for i=3:length(names)
    ...
    out_l=mean(diff_ac)-2*std(diff_ac);
    out_h=mean(diff_ac)+2*std(diff_ac);
    ind_in=diff_ac>out_l & diff_ac<out_h;
    sub_in(1,i-2)=mean(ind_in);
   
    mean_base(i-2)=mean(diff_ac(ind_in))*convert_fact;
    ...
    out_h=mean(diff_ac)+2*std(diff_ac);
    ind_in=diff_ac>out_l & diff_ac<out_h;
    sub_in(2,i-2)=mean(ind_in);
    
    mean_cond(i-2)=mean(diff_ac(ind_in))*convert_fact;
end
